...
* **后端与 BLS 节点**：通过 API 进行通信，后端将待签名的 UserOperation 哈希发送给 BLS 节点，BLS 节点返回 BLS 签名。BLS节点本身应该是一个permissionless接入的，即调用它的外部服务不需要任何身份认证，因为这个节点模块仅仅负责bls签名及验签，但bls节点的创建，是需要往链上合约注册的，即只有合约中注册的bls节点才允许进行签名
